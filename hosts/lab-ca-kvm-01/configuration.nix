...
in
{
  ...
  networking.hostName = "lab-ca-kvm-01";

  # Set NIX_PATH for NixOS config and nixpkgs.
  nix.nixPath = [
    "nixpkgs=/nix/var/nix/profiles/per-user/root/channels/nixos"
    "nixos-config=/etc/nixos/hosts/lab-ca-kvm-01/configuration.nix:/nix/var/nix/profiles/per-user/root/channels"
  ];
  ...
  boot.loader.grub.mirroredBoots = [
    {
      devices = [ "/dev/disk/by-uuid/B8E8-040F" ];
      path = "/boot-fallback";
    }
  ];
  ...
  networking.interfaces.br0.macAddress = "70:85:c2:62:2d:40";
  networking.interfaces.br0.ipv4.addresses = [{
    address = "172.30.194.5";
    prefixLength = 24;
  }];
  ...
  boot.initrd.network = {
    ...
    # `boot.initrd.kernelModules`, so your initrd can load it!
  ...
  services.apcupsd.configText = ''
    ...
    NETSERVER on
    NISIP 0.0.0.0
    # Shutdown the system when the battery level is below 20 percent.
    BATTERYLEVEL 20
  '';
  # Allow access to `apcupsd`.
  ...
  boot.kernelParams = [
    ...
    "ip=172.30.194.5::172.30.194.1:255.255.255.0:lab-ca-kvm-01:enp0s31f6"
    # Enable IOMMU groups. Use "amd_iommu=on" on an AMD system.
    "intel_iommu=on"
  ];
  ...
  #
  # In our case, the kernel module is `e1000e`.
  #
  ...
  # `kvm-amd` module.
  boot.initrd.kernelModules = [ "e1000e" "kvm-intel" ];
  ...
  # Enable cron on the system for scheduling repetitive tasks.
  services.cron.enable = true;
  services.cron.systemCronJobs = [
    # Add a cron job to disable power management on the ethernet adapter on every boot.
    "@reboot root ${pkgs.ethtool}/sbin/ethtool -K enp0s31f6 gso off tso off gro off"
  ];
  ...
  services.syncoid =
    ...
    in
    {
      ...
      #commonArgs = [ "--no-resume" ];
      commands = {
        # Remote syncs.
        "zroot-3z8b5j/safe" = mkRemoteSync "zroot-3z8b5j/safe";
      };
    };
